...
// 🔄 ফায়ারবেস থেকে রিয়েলটাইমে ডেটা পাওয়ার ফাংশন
async function fetchFirebasePanelData(panelType) {
    try {
        ...
        const options = Object.keys(data)
            ...
            .map(key => ({
                label: typeof data[key] === "string" ? data[key] : key,
                value: key
            }));
        ...
        return { options, customTitle, customDescription, customImage, fullData: data };
    } catch (error) {
...
// ================================
// ⚡ PART 3 - Interactions & Dynamic Panels
// ================================
...
// ================================
// ⚡⚡ REALTIME FIREBASE SYNC (NO CODE CHANGE REQUIRED)
// ================================

function initFirebaseRealtimeSync(guild) {
    // 1. Listen for Panel Changes in Firebase Database
    db.ref("panels").on("value", async (snapshot) => {
        const panelsData = snapshot.val() || {};
        console.log("⚡ Firebase Panels updated in Realtime!");

        for (const [panelKey, channelId] of Object.entries(CHANNELS)) {
            const channel = guild.channels.cache.get(channelId);
            if (!channel) continue;

            const panelType = panelKey.toLowerCase().replace("_panel", "");
            const pData = panelsData[panelType] || {};

            const embed = new EmbedBuilder()
                .setTitle(pData.title || `✨ ${panelType.toUpperCase()} PANEL`)
                .setDescription(pData.description || "নিচের অপশন থেকে নির্বাচন করুন।")
                .setColor("Blue");
            ...
            if (options.length > 0) {
                ...
                try {
                    ...
                    console.error("Realtime Panel Update Error:", err);
                }
            }
        }
    });

    // 2. Verification Link Realtime Tracking
...
client.on("ready", async () => {
    ...
    if (guild) {
        ...
        initFirebaseRealtimeSync(guild); // Start Realtime Tracking
    }
});
